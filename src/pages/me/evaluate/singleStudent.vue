<template>
    <div class="evaluate-container">
        <!-- <div class="top">
            <div class="tit">家长<span>高吸</span>的评价</div>
            <div class="conetnt">老师讲课很有耐心！</div>
            <div class="view-img">
                <img class="img" v-for="img in imgs" :src="img" :key="img">
            </div>
        </div> -->

        <div class="top mt20">
			<div class="tit">对学员<span>{{studentName}}</span>的评价</div>
			<div class="text-area">
...
export default {
	...
	methods:{
		onLoad(e){
			this.form.studentLessonId = e.studentLessonId;
		...
		submit(){
			...
			uni.request({
				method: 'POST',
				url: `${this.doMain}/evaluation/addEvaluationV2`,
				header: {
				...
				success: res => {
					if (res.data.code === 0) {
						...
						uni.navigateTo({
							url: '../../me/evaluate/chooseStudent?lessonId='+this.lessonId
						});
					}else{
						uni.showToast({
