...
export default {
    ...
    methods: {
			...
			uni.request({
				...
				success: res => {
					...
					if (res.data.code === 0) {
						...
						this.payedAmount = res.data.data.totalAmount/100;
					}else{
						uni.showToast({
		...
		submit() {
			...
			uni.request({
			    method: 'POST',
			    url: `${this.doMain}/order//orderAgain/topay`,
			    header: {
			        'content-type': 'application/x-www-form-urlencoded'
			    },
			    data: { orderId: this.orderId,payAmount:this.payedAmount, payType:this.payType,isJiaoNa:this.isPay},
			    success: res => {
					uni.hideLoading();
			        if (res.data.code === 0) {
