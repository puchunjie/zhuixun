...
export default {
    ...
    methods: {
		...
		getClassCourseList() {
			uni.request({
			    method: 'POST',
			    url: `${this.doMain}/order/paike/view`,
			    header: {
		...
		submit(){
			...
			uni.request({
			    method: 'POST',
			    url: `${this.doMain}/order/paike`,
			    header: {
			        'content-type': 'application/x-www-form-urlencoded'
			    },
			    data: { orderId: this.orderId,teacherId:this.userinfo.teacherId,classId:this.classId,courseIdsStr:courseIds,lessonNumsStr:lessonNums,studentId:this.studentId},
			    success: res => {
			        if (res.data.code === 0) {
						uni.showToast({
							title:'排课成功',
